...
> *Setup:* PlaySound-Node im Graph auswählen. Details-Panel rechts zeigt: `Sound = SFX_Heartbeat`, `VolumeMultiplier = 0.4`, `PitchMultiplier = 1.0`, `NodeAudioMode = Force2D`. Alle Felder ausgefüllt, roter Pfeil auf `VolumeMultiplier`.
...
| `NodeAudioMode` | EMayDialogueAudioMode | `Default` / `Force2D` / `Spatial3D` — der Tri-State-Modus, der das alte `bForce2D`-Bool abgelöst hat |
| `AttenuationOverride` | USoundAttenuation | Node-spezifische Attenuation; nur im `Spatial3D`-Modus aktiv |
