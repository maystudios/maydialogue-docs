...
> *Setup:* PlayerChoice-Node ausgewählt, Choice 4 im Details-Panel ausgeklappt. `ChoiceText = "Gilden-Rabatt bitte."`, darunter Requirements: `HasTag: RequiredTag = Guild.Member.Active, bCheckOnInstigator = true, FailResult = FailedAndHidden`.
...
`FailResult = FailedButVisible` statt `FailedAndHidden`. Oder `RequiredTag` hat Tippfehler – das Requirement greift ins Leere und gibt `Passed` zurück.
