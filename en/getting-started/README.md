...
MayDialogue is a dialogue plugin for Unreal Engine 5.7. You get a visual graph editor, a ready-made UI layer, 3D audio with a procedural Babel fallback, camera controls, typewriter effects, GAS integration, and an in-editor preview — all in a single plugin, with no external dependencies.
...
{% hint style="warning" %}
**Blueprint-only project?** You can use the plugin as a binary distribution without compiling it yourself. As soon as you write custom nodes, requirements, or side effects in C++, your project will need a C++ module and a rebuild.
{% endhint %}
